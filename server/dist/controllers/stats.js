...
export const getDashboardStats = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key))
        ...
        const [thisMonthProducts, lastMonthProducts, thisMonthUsers, lastMonthUsers, thisMonthOrders, lastMonthOrders, productCount, userCount, allOrders, lastSixMonthOrders, categories, maleUsersCount, femaleUsersCount, latestTransaction] = await Promise.all([
...
export const getPieStats = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key)) {
        ...
        const [processingOrders, shippedOrders, deliveredOrders, categories, productCount, productsOutOFStock, allOrders, allUsers, adminUsers, customerUsers] = await Promise.all([
    ...
    return res.status(200).json({
        success: true,
        charts,
        message: "PieChart's stats fetched successfully"
    });
});
export const getBarStats = TryCatch(async (req, res, next) => {
    ...
    return res.status(200).json({
        success: true,
        charts,
        message: "barChart's stats fetched successfully"
    });
});
export const getLineStats = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key)) {
        ...
        const twelveMonthsAgo = new Date();
        twelveMonthsAgo.setMonth(twelveMonthsAgo.getMonth() - 12);
        const lastTwelveProductPromise = Product.find({
            createdAt: { $gte: twelveMonthsAgo, $lte: today }
        }).select("createdAt");
        const lastTwelveMonthUsersPromise = User.find({
            createdAt: { $gte: twelveMonthsAgo, $lte: today }
