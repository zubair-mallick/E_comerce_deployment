...
const Skeleton = ({ className }) => (<div aria-live="polite" aria-busy="true" className={`skeleton ${className}`}>
    <span className="skeleton__pulse"></span>
    <br />
  </div>);
const SVGSkeleton = ({ className }) => (<svg className={`skeleton-svg ${className} animate-pulse`}></svg>);
const ProductTableSkeleton = () => (<div className="product-table-skeleton">
    <table>
      <thead>
        <tr>
      ...
      <tbody>
        {Array.from({ length: 20 }).map((_, index) => (<tr key={index}>
